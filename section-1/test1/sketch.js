...
function drawViz(){
    for(let i = 0; i < treedata.length; i++){
        let nSize = map(treedata[i]['RawRingWidth_mm'], 0, 1.5, 25,400);
        push();
        translate(width/2,height/2);
        stroke(0,30);
        noFill();
        ellipse(0,0,nSize,nSize);
        pop();
    }
}
...
function drawName(){
    for(let i = 0; i < treedata.length; i++){
        let nSize = map(treedata[i]['RawRingWidth_mm'], 0, 1.5, 25,400);
   //console.log(mouseX); 
        if(mouseX < width/2 + nSize/2+2&&mouseX>width/2+nSize/2-2||mouseX < width/2 - nSize/2+2&&mouseX>width/2-nSize/2-2){
            //console.log(treedata[i].year, treedata[i]['RawRingWidth_mm'], nSize)
            //console.log(mouseX, nSize, treedata[i].year);
            text(treedata[i].year + ' - ' + treedata[i]['RawRingWidth_mm'], width-100, 100);
            push();
            noFill();
            strokeWeight(4);
            stroke(255,0,0);
            ellipse(width/2, height/2, nSize, nSize);
            pop();
        }
    }
}
